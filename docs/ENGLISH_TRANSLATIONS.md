...
2. The simulated student gives a correct answer and a short reason.
...
5. After every reply, update the set of candidate misconceptions that still
   appear plausible.
6. Choose one final diagnosis.
7. The server scores the exact diagnosis and candidate-set history against the
   pre-authored answer key.
8. Model-generated coaching is shown separately from the deterministic score.

The first scenario uses the misconception that multiplication always makes a
number larger. A student can answer `3 × 5 = 15` correctly while still applying
that mistaken general rule. A discriminating question such as asking about
`1/2 × 8` can expose the boundary error. The second scenario will cover one
well-sourced middle-school science misconception; the MVP will not generate
arbitrary subjects.
...
return a schema-constrained server state. The Responses API, GPT-5.6, and
Structured Outputs provide that dynamic behavior.

GPT-5.6 does not own the answer key or score. Each scenario stores a human-
authored misconception ID, immutable belief state, candidate diagnoses, and
...
candidate updates and the fixed answer key. Model-generated coaching is labeled
...
the core flow works by keyboard, a round finishes in 90 seconds, and at least
one real teacher or tutor tests the experience. Misconception faithfulness is
evaluated separately with fixed targeted, misaligned, generic, and injection
probes plus human-authored pass/fail criteria. Model self-evaluation is not
...
invalid schema gets one retry and then an explicit failure state.
...
90-second sample round. A separate human-labeled fixed probe suite checks
misconception faithfulness without using the same model as a judge.
